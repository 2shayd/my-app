...
const EntryCard: React.FC<Entry> = ({ id, startDate, startTime, endDate, endTime, scaleRating, pinned }) => {
            ...
            <Card variant="filled" className="mt-4">
                ...
                >
                    <Icon
                        as={StarIcon}
                        size='xl'
                        className={`${pinned ? 'text-blue-500' : 'text-gray-500'} abosulte right-4 top-4`}    
                    />
                </Pressable>
                <Text className='text-md my-1 dark:text-white'>{startDate}</Text>
                <Text className='text-md my-1 dark:text-white'>Pain rating: {scaleRating}</Text>
                <Link
